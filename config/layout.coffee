module.exports =
    logo: "http://drive.google.com/uc?export=view&id=0B7bv1IFIZGRbbzZCQUg5WUEwejQ"
    message: "Come to ECS 116 at 7:30pm on September 21st for our General Meeting. All students are welcome!"
    intro:
        logo: "/images/aero_logo.png"
        background: "http://drive.google.com/uc?export=view&id=0B7bv1IFIZGRbWEUxa0RramtIWDg"
        buttons: [
            title: 'twitter'
            fa: 'twitter'
            href: 'https://twitter.com/uvicaero'
        ,
            title: 'facebook'
            fa: 'facebook'
            href: 'https://www.facebook.com/groups/uvicaero/'
        ,
            title: 'youtube'
            fa: 'youtube'
            href: 'https://www.youtube.com/user/aeroUVic'
        ]
    navbarRight: [
        title: 'About'
        href: '/about'
    ,
        title: 'Join'
        href: '/#join'
    ,
        title: 'Current Exec'
        href: '/currentexec'
    ,
        title: 'Sponsorship'
        dropdown: [
            title: "Current Sponsors"
            href: "/sponsors"
        ,
            title: "Sponsor Us"
            href: "/sponsorus"
        ,
            title: "Donate"
            href: "/sponsorus/#donate"
        ]
    ,
        title: 'Mailing List'
        href: '/mail'
    ,
        title: 'Contact'
        href: '/contact'
    ]
    footer: [
    ]
